...
// const User = require('./User'); // Required for embedding example
	...
	priceDiscount: {
		...
		validate: {
			validator: function (priceDiscount) {
				...
				return priceDiscount < this.getUpdate().$set.price;
			},
...
tourSchema.pre(/^find/, function (next) {
	this.populate({
		path: 'guides',
		select: '-__v -passwordChangedAt -passwordResetExpires -passwordResetToken -createdAt - -_id'
	});
